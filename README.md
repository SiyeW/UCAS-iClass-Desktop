...
当前版本提供课程查询、动态签到二维码、手动签到主流程，以及可选的本机加密登录信息保存。系统托盘、后台自动签到、计划任务、开机启动与通知尚未实现。
...
- 按学校时间轴生成、自动刷新签到二维码
- 对选中课程手动提交签到
- 默认情况下，账号、密码和会话仅在当前进程内存中保存；关闭程序后即丢弃
- 可选“在此 Windows 账户中记住登录”：登录成功后，使用 Windows DPAPI 加密保存账号和密码；可在程序内随时清除
...
- 请勿在他人电脑上输入或保存学校账号密码。
- 默认不将凭据、会话或课程内容写入磁盘。
- 若主动勾选“在此 Windows 账户中记住登录”，账号和密码会用 Windows DPAPI 加密保存于当前 Windows 账户的 `%LOCALAPPDATA%\UCASiClassDesktop\credentials.dpapi`；该文件不保存会话，可在程序内清除。
...
请先阅读 [`docs/DEVELOPMENT.md`](docs/DEVELOPMENT.md)。凭据持久化仅限明确选择、仅限本机 Windows 账户的 DPAPI 方案；在真实学校账号完成手动签到流程验证前，不应实现后台自动签到或计划任务。
